...
function Widget(props) {
    ...
    kpisSrvs
      ...
      .then((response) => {
        ...
        for (let { name, value } of response) {
          if (name === "none") name = t("kpisApp:others");
          d.labels.push(name);
  ...
  return (
    <Card
      className="w-full rounded-8 shadow-none border-1 p-8 flex flex-wrap flex-col justify-between"
      style={{ opacity: loading ? 0.6 : 1 }}
    >
      <div>
        <Typography className="h3 sm:h2">
          {t(`kpisApp:${math}_titles.by_clients`)}
        </Typography>
      </div>
