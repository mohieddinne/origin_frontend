...
function FilterMenu(props) {
  ...
  const isadmin = user?.data?.level?.name === 1;
  ...
  const resetFilters = () => {
    dispatch(Actions.resetFilters(isadmin));
  };
    ...
    >
      ...
      <MenuItem>
        <FormControl className="w-full">
          ...
          <Select
            disabled={filtersData?.users?.length < 2}
            ...
            multiple
          >
            {filtersData?.users?.map(
              ({ name, value, actif, id }, key) => {
                console.log({ name });
                return (
      ...
      <MenuItem className="flex flex-wrap">
        ...
        <MuiPickersUtilsProvider utils={DateFnsUtils}>
          <KeyboardDatePicker
            ...
            className="w-1/2"
            style={{ paddingRight: "12px" }}
            label={t("activities:date_facturation_start")}
            format="yyyy-MM-dd"
            value={filters.date_facturation_start || null}
            InputProps={{
              startAdornment: filters.date_facturation_start && (
                <IconButton
                  onClick={() =>
                    handleDateChange("date_facturation_start", null)
                  }
                  disabled={!filters.date_facturation_start}
                  style={{ order: 1 }}
                >
                  <CloseIcon />
                </IconButton>
            ...
            onChange={(date) =>
              handleDateChange("date_facturation_start", date)
            }
          />

          <KeyboardDatePicker
            autoOk
            label={t("activities:date_facturation_end")}
            format="yyyy-MM-dd"
            className="w-1/2"
            value={filters.date_facturation_end || null}
            InputProps={{
              startAdornment: filters.date_facturation_end && (
                <IconButton
                  onClick={() =>
                    handleDateChange("date_facturation_end", null)
                  }
                  disabled={!filters.date_facturation_end}
                  style={{ order: 1 }}
                >
                  <CloseIcon />
                </IconButton>
            ...
            InputAdornmentProps={{ position: "end" }}
            onChange={(date) =>
              handleDateChange("date_facturation_end", date)
            }
          />
        </MuiPickersUtilsProvider>
      </MenuItem>
