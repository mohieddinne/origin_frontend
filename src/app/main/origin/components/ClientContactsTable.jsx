...
import ContactSubRow from "./ClientContactCard";
...
function ClientContactsHandler(props) {
  const { folderIds, clientIds } = props;
  const variables = {};
  const filters = [];

  if (Array.isArray(folderIds) && folderIds.length) {
    filters.push({ name: "folders", value: folderIds });
  }
...
function ClientContactsTable({ variables }) {
  const { t } = useTranslation();
  ...
  return (
    <EnhancedTable
      ...
      options={{
        refetch,
        subRow: (row) => <ContactSubRow id={row.original.id} />,
        special_filter: true,
      }}
    />
  );
}
export default ClientContactsHandler;
